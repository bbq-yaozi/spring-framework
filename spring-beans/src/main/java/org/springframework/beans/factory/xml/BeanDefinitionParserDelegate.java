...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		String beanName = id;
		// id为空，但是name不为空的时候，取alias的第一个作为beanName。
		...
		// 进一步解析其他属性并封装到GenericBeanDefinition类型的实例中
		AbstractBeanDefinition beanDefinition = parseBeanDefinitionElement(ele, beanName, containingBean);
		if (beanDefinition != null) {
			if (!StringUtils.hasText(beanName)) {
				try {
					// 如果不存在beanName，那么根据Spring中提供的规则为当前bean生成对应的beanName。
					if (containingBean != null) {
			...
			// 将获取的信息封装到BeanDefinitionHolder中。
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		/**
		 * <bean id="aaa" class="com.xxx.AAA">
		 * 		<meta key="test" value="aaaaaa" />
		 * </bean>
		 */
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		if (hasRefAttribute) {
			...
			// value属性的处理，使用TypedStringValue封装对应的value
